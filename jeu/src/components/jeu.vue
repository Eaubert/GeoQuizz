    ...
    <div>
        <div id="mapid" v-on:click.capture="onMapClick">
            <v-map :zoom=13 :center="[jeu.mlat ,jeu.mlng ]" ref="map">
                <v-tilelayer
                        url="https://api.tiles.mapbox.com/v4/mapbox.streets/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoibmV4emlvIiwiYSI6ImNqZDI0Z2xsejV0ZTMycXJvZG9tMGE0bjIifQ.CHThkqzyGXGNhCh9KRnUpw"></v-tilelayer>
    ...
    created () {
      ...
      api.get('/parties/' + this.$route.params.id + '/photos?token=' + sessionStorage.getItem('user'))
        .then((response) => {
          ...
          var fn = this.images.photos[0].photo.url;
          if (fn.substring(0, 4) == 'http') {
            jeu.url = fn
          } else {
            jeu.url = config.url + 'file/download/' + fn
          }
        }).catch(function (error) {
      ...
      onMapClick (e) {
        if (this.len < 10) {
          ...
          if (this.len != 10) {
            ...
            var fn = this.images.photos[this.len - 1].photo.url
            if (fn.substring(0, 4) == 'http') {
              jeu.url = fn
            } else {
              jeu.url = config.url + 'file/download/' + fn
